 ...
 *
 * The vmm segment driver was designed for mapping regious of kernel memory
...
segvmm_create(struct seg *seg, void *argsp)
{
	...
	data->svmd_kaddr = (uintptr_t)a->kaddr;
	data->svmd_prot = a->prot;
	data->svmd_cookie = a->cookie;
	data->svmd_hold = a->hold;
	data->svmd_rele = a->rele;
...
{
	segvmm_data_t *svmd = (segvmm_data_t *)seg->s_data;
...
{
	segvmm_data_t *data = (segvmm_data_t *)seg->s_data;
...
segvmm_fault_in(struct hat *hat, struct seg *seg, uintptr_t va, size_t len)
{
	...
	/* XXX: Make simple for regular pages */
	va &= PAGEMASK;
	...
	do {
		...
		/*
		 * XXX: handle larger pages, and all the danger that entails
		 * psz = LEVEL_SIZE(lvl);
		 * map_addr = va & LEVEL_MASK(lvl);
		 */
		psz = PAGESIZE;
    ...
    enum fault_type type, enum seg_rw tw)
{
	...
	/* XXX: softlock handling is probably messed up */
	if (type == F_INVAL ||
	    (type == F_SOFTLOCK && svmd->svmd_softlockcnt == 0)) {
