 ...
 * Locking strategy
 *
 * The global vmm_zsd_lock is held when while modifying vmm_zsd_list.
 *
...
vmm_zsd_add_vm(vmm_softc_t *sc)
{
	...
	/*
	 * XXX-mg
	 * The create callback should have been called on all existing zones.
	 * Not sure about the global zone, so this may panic.
	 */
	VERIFY(zsd != NULL);
...
vmm_zsd_shutdown(zoneid_t zid, void *data)
{
	...
	for (sc = list_head(&zsd->vz_vmms); sc != NULL;
		...
		/* Send an poweroff to the VM, whether running or not. */
