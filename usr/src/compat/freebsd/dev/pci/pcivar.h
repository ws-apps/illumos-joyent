...
pci_msi_count(device_t dev)
{
	...
	if (ddi_intr_get_nintrs(dip, DDI_INTR_TYPE_MSI, &num) != DDI_SUCCESS)
		num = 0;
...
static inline int
pci_alloc_msi(device_t dev, int *num)
{
	dev_info_t *dip;

	if (ddi_intr_get_navail(dip, DDI_INTR_TYPE_MSI, num) != DDI_SUCCESS) {
		*num = 0;
		return (EINVAL);
	}
