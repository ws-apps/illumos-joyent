...
static int devfs_ppt_ln(di_minor_t minor, di_node_t node);
 ...
 */
static devfsadm_create_t devfs_ppt_create_cbt[] = {
	{ "pseudo", "ddi_pseudo", NULL, TYPE_EXACT, ILEVEL_0, devfs_ppt_ln }
};

DEVFSADM_CREATE_INIT_V0(devfs_ppt_create_cbt);

static int
devfs_ppt_ln(di_minor_t minor, di_node_t node)
{
	char *mn;

	if (strcmp(di_driver_name(node), "ppt") != 0)
